...
{
    return [UIImage scaleImage:image toSize:size scale:[[UIScreen mainScreen]scale]];
}

+ (UIImage*)scaleImage:(UIImage *)image toSize:(CGSize)size scale:(NSInteger)scale
{
    ...
    if (image.size.width > size.width || image.size.height > size.height) {
        ...
        [scaleImage drawInRect:CGRectMake(0, 0, w, h)];
...
//改变图片颜色
- (UIImage *)imageWithColor:(UIColor *)color ahple:(CGFloat)ahple
{
    ...
    //设置透明度(值可根据需求更改)
    CGContextSetAlpha(context, ahple);
    //设置混合模式
